    ...
    public class DAProfesor
    {
        ...
        public int InsertarDatosBD(EntidadProfesor profesor)
        {
            ...
            string sentencia = "INSERT INTO Profesores(Nombre_Profesor, Primer_Apellido_Prof, Segundo_Apellido_Prof," +
                               ...
                               "Curso_Certificado, Hora_Entrada,Hora_Salida, Estado, Borrado) VALUES(@NOMBRE, @PRIMER_APELLIDO," +
                               "@SEGUNDO_APELLIDO, @FECHA_NACIMIENTO, @TELEFONO, @CORREO, @CUENTA, @DIRECCION, @CURSO_CERTIFICADO, @HORA_ENTRADA," +
                               "@HORA_SALIDA, @ESTADO, @BORRADO) SELECT @@IDENTITY";
            ...
            comando.Parameters.AddWithValue("@ESTADO", profesor.getEstado());
            comando.Parameters.AddWithValue("@BORRADO", profesor.getBorrado1());
        ...
        public EntidadProfesor ObtenerProfesor(int id)
        {
            ...
            string sentencia = string.Format("SELECT ID_Profesor, Nombre_Profesor, Primer_Apellido_Prof, Segundo_Apellido_Prof," +
                " Numero_Telefonico, Correo_Electronico_Prof,Direccion_Exacta  FROM Profesores WHERE ID_Profesor = {0}", id);
            ...
            try
            {
                ...
                if (dataReader.HasRows)
                {
                    ...
                    profesor.NumeroTelefono = dataReader.GetString(4);
                    profesor.CorreoElectronico = dataReader.GetString(5);
                    profesor.DireccionExacta = dataReader.GetString(6);
                }
                conexion.Close();
            }
            catch (Exception)
        ...
        public int ActualizarProfesor( EntidadProfesor profesor)
        {
            ...
            string sentencia = "UPDATE Profesores SET Nombre_Profesor = @NOMBRE_PROFESOR, Primer_Apellido_Prof = @PRIMER_APELLIDO," +
                ...
                " Curso_Certificado = @CURSO_CERTIFICADO, Hora_Entrada = @HORA_ENTRADA, Hora_Salida = @HORA_SALIDA, Estado = @ESTADO, " +
                "Borrado = @BORRADO  WHERE ID_Profesor = @ID_PROFESOR ";
            cmd.CommandText = sentencia;
            cmd.Connection = conexion;
            cmd.Parameters.AddWithValue("@ID_PROFESOR", profesor.IdProfesor);
            cmd.Parameters.AddWithValue("@NOMBRE_PROFESOR", profesor.NombreProfesor);
